...
### 手順
1. コンテンツを取得（以下の優先順位で）:
   1. まず対象ページの **RawContent** プロパティを確認する
   2. RawContent に値がある場合はそれをコンテンツとして使用（外部アクセス不要）
   3. RawContent が空の場合のみ、SourceURL から取得を試みる:
      - Web記事: `WebFetch` ツールで取得
      - YouTube: URLからメタデータ取得（トランスクリプトは取れない場合あり）
   4. SourceURL の取得がセキュリティ制限等でブロックされた場合はエラーとせず処理を継続する。RawContent も空の場合のみユーザーに手動入力を求める。
2. コンテンツを分析して以下を生成:
...
| SourceURL アクセス不可（セキュリティブロック含む） | **RawContent を代替ソースとして使用し処理を継続**。エラーとして扱わない。RawContent も空の場合のみユーザーに確認 |
